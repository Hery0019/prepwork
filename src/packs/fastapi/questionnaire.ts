// Le questionnaire du pack `fastapi` (ADR 0011 §9) : ordre fixe, valeurs par défaut entre
// crochets, réponses validées, puis récapitulatif et confirmation. Une question dont la réponse
// est unique n'est pas posée : elle est annoncée. La version de Python, uv, ruff, mypy, l'ORM et
// l'outil de migration ne sont jamais demandés.
...
import { PINNED_VERSIONS } from './context.js';
import {
  defaultPackageName,
  packageNameProblem,
  ScaffoldSchema,
...
export interface FastapiQuestionnaireResult {
  scaffold: Scaffold;
...
function summary(scaffold: Scaffold): string {
  return [
    ...
    `Paquet         ${scaffold.project.package_name}`,
    `Python         ${PINNED_VERSIONS.python} (épinglé par l'outil)`,
...
export async function runQuestionnaire(
...
): Promise<FastapiQuestionnaireResult> {
  ...
  const packageName = await prompter.text({
    message: '2. Nom du paquet Python (snake_case)',
    initialValue: defaultPackageName(name),
    validate: packageNameProblem,
  });
  const description = (
    await prompter.text({
  ...
  // 4. Base de données. Alembic est le seul outil de migration : aucun choix à faire (ADR 0011 §5).
  const database = await prompter.select<Database>({
    message: '4. Base de données',
    options: [
      { value: 'postgresql', label: 'PostgreSQL', hint: 'asyncpg' },
      { value: 'mysql', label: 'MySQL', hint: 'asyncmy' },
  ...
  // 6. Sécurité, 6a. émetteur des jetons
  const security = await prompter.select<Security>({
    ...
    options: [
      ...
      { value: 'session', label: 'Session par cookie', hint: 'front servi depuis le même site' },
      {
        value: 'oauth2-resource-server',
        label: 'Jetons OAuth2',
        hint: 'fournisseur OIDC externe',
      },
    ],
    initialValue: 'none',
  });
  ...
  if (security === 'oauth2-resource-server') {
    const issuer = await prompter.text({
      message: '6a. Émetteur OIDC (va dans .env.example, pas dans scaffold.yaml)',
      placeholder: 'https://auth.example.com/realms/app',
      validate: validateIssuer,
    });
    extras.envOverrides = { OIDC_ISSUER: issuer };
  }
  ...
  const scaffold = ScaffoldSchema.parse({
    ...
    project: { name, package_name: packageName.trim(), description },
