...
|---|---|
| Nom | `pay-flow` |
| Espace de noms racine | `Solumada.PayFlow` |
| .NET | 10 |
| Base de données | PostgreSQL |
| Profil d'architecture | `layered` — Monolithe en couches — Api, Application, Infrastructure, Domain, un projet chacun. |
| Sécurité | `none` |
| Docker | oui |
| CI | GitHub Actions |
...
- **CORE-040** — Les identifiants du code (espaces de noms, classes, méthodes, variables, colonnes) sont toujours en anglais.
...
Un skill par sujet. Chaque règle y porte un identifiant stable (`NET-002`) à citer dans les plans et les revues.

| Skill | Contenu | Fichier |
|---|---|---|
| `architecture` | Projets, espaces de noms, sens des dépendances et exemple de référence du projet. À lire avant de créer une classe ou d'ajouter une référence. | `.claude/skills/architecture/SKILL.md` |
| `db` | Entités, `DbContext`, dépôts, transactions et migrations EF Core. À lire avant de toucher au schéma ou à la persistance. | `.claude/skills/db/SKILL.md` |
| `api` | Contrôleurs, DTO, `ProblemDetails`, pagination et versionnement. À lire avant d'exposer ou modifier un endpoint. | `.claude/skills/api/SKILL.md` |
...
| `security` | Secrets, points de terminaison de santé, CORS, en-têtes, analyse des dépendances et option de sécurité du projet. À lire avant de toucher à la configuration ou à l'authentification. | `.claude/skills/security/SKILL.md` |
