...
Pipeline GitLab CI qui exécute le build, les trois niveaux de test et le contrôle des migrations.

### Règles

- **CIGL-001** · `format` — Le pipeline exécute `dotnet format --verify-no-changes`, `dotnet build` et `dotnet test` à chaque push et merge request.
  Pourquoi : La CI doit exécuter exactement ce qu'un développeur peut lancer localement, ni plus ni moins.
- **CIGL-002** · guidance — Les tests d'intégration ont besoin d'un démon Docker ; le job déclare le service `docker:dind` plutôt que de les ignorer.
  Pourquoi : Ignorer le niveau qui touche la vraie base supprime le seul test qui prouve le SQL.
- **CIGL-003** · guidance — Un pipeline rouge bloque la fusion ; rien n'est fusionné avec un job en échec.
