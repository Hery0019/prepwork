...
Pipeline GitLab CI qui exécute les vérifications, les trois niveaux de test et le contrôle des migrations.

### Règles

- **CIGL-001** · `format` — Le pipeline exécute `ruff format --check`, `ruff check`, `mypy`, `lint-imports` et `pytest` à chaque push et merge request.
  Pourquoi : La CI doit exécuter exactement ce qu'un développeur peut lancer localement, ni plus ni moins.
- **CIGL-002** · guidance — `lint-imports` est un job à part entière, jamais fondu dans le job de lint.
  Pourquoi : C'est la seule chose qui tient l'architecture ; un échec doit nommer le contrat de couches, pas « lint failed ».
- **CIGL-003** · guidance — Le niveau intégration tourne via le service Docker-in-Docker ; sans lui le job est rouge, jamais silencieusement ignoré.
  Pourquoi : Un niveau ignoré ressemble exactement à un niveau réussi dans le résumé du pipeline.
- **CIGL-004** · guidance — Un pipeline rouge bloque la fusion ; rien n'est fusionné avec un job en échec.
