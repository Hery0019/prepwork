"""Core rules that the `import-linter` contract cannot express.

Each test carries the identifier of the rule it proves, like an
ArchUnit test on the Spring side: this is the convention the
catalogue consistency check verifies.
"""
...
from edge_api.api.errors import PROBLEM_MEDIA_TYPE
from edge_api.api.settings import Settings
from edge_api.domain.exceptions import ConflictError, NotFoundError
...
def test_CORE_013_no_route_exposes_a_domain_entity() -> None:
    """Stated on the module, not the ORM: the rule holds without a database too."""
    domain = "edge_api.domain"
    for route in _app().routes:
        ...
        for annotation in annotations:
            ...
            assert not module.startswith(domain), f"{route.path} exposes the domain"
...
def test_CORE_021_the_test_database_is_not_sqlite() -> None:
    """The Python counterpart of the H2 ban: SQLite does not know the production SQL."""
    import os
...
def test_CORE_023_every_test_carries_exactly_one_level_marker() -> None:
    """Enforced by the pytest configuration and the hook below; this test documents the rule."""
