...
- **NEXT-001** · guidance — Un composant est un composant serveur par défaut ; `'use client'` n'est ajouté que pour un état, un effet, un gestionnaire d'événement ou une API du navigateur.
  Pourquoi : Chaque composant client envoie du JavaScript au navigateur ; le défaut doit être le moins cher.
- **NEXT-011** · guidance — Une page déclare ses états de chargement et d'erreur avec `loading.tsx` et `error.tsx` à côté d'elle.
  Pourquoi : Le framework diffuse déjà les deux ; les écrire à la main dans la page perd ce bénéfice.
